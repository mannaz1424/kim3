...
tApp.factory("TService", function($http){
	...
	var service = {
			...
			modifyPost: modifyPostAjax,
			updatePost: updatePostAjax
	...
	function modifyPostAjax(param){
		return $http.post("/selectModify", param).then(handleSuccess, handleError);
	}
	
	function updatePostAjax(param){
		return $http.post("/updateContent", param).then(handleSuccess, handleError);
	}
